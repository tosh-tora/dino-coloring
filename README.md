...
- レベル判定は下絵を 1024x768 → 256x192 に縮小したアルファだけを見る軽い方法。「塗る領域の数」（線でない画素の連結成分）と「線の密度」（線画素の割合）を取り、**難しい方**を採る。領域数だけだと草の穂先のような開いた細線だらけの絵が簡単側に落ちるため。同梱 64 点の実測（単体恐竜=領域 2〜9/密度 6〜14%、細密=領域 25〜52 または密度 27〜37%）からしきい値を決めており、64 点が 21/24/19 に分かれる。1 点 3ms 程度で、結果は IndexedDB (`levels`) にキャッシュする。`src/level.ts` の `LEVEL_VERSION` を上げると作り直される
