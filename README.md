...
- 領域の"数"は使わない。大きな葉が並ぶだけの素朴な絵（ヴェロキラプトル）が難しい側に、細い線だらけの絵（フクイベナートル）が簡単側に落ちてしまうため。しきい値は同梱の下絵の実測と目視ラベルから決めており、67 点が 26/16/25 に分かれる。結果は IndexedDB (`levels`) にキャッシュするので、判定が走るのは初回だけ（Chrome で 1 点 10ms 弱・67 点で 0.5 秒ほど）。`src/level.ts` の `LEVEL_VERSION` を上げると作り直される
